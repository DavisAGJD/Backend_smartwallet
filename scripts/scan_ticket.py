...
# Configuramos un batch size mayor si hay GPU disponible
...
def str_to_float(s: str) -> Optional[float]:
    ...
    try:
        ...
        # Si hay coma y punto, se asume formato europeo (ej. 1.234,56)
        ...
        # Eliminar puntos de miles: unir todas las partes excepto la última (decimal)
...
def procesar_imagen_pil(ruta_imagen: str) -> str:
    ...
    Pipeline de preprocesamiento usando PIL:
      ...
      - Redimensiona la imagen a un ancho base.
...
def detectar_tienda(texto: str) -> str:
    ...
    # Primera estrategia: buscar patrones en las primeras líneas
    patrones = {
    ...
    # Segunda estrategia: búsqueda por keywords exactos
    keywords = {
        'AKI': 'Super Aki',
        'AKP': 'Super Aki',
        'AKT': 'Super Aki',
    ...
    # Tercera estrategia: fuzzy matching en cada línea
...
def extraer_total_estrategias(texto: str) -> Optional[float]:
    ...
    texto_norm = normalizar_texto(texto)
    estrategias = [
        # Busca "TOTAL" (con posibles variantes) seguido de un número
        lambda: re.search(r'T[O0O]+T[A4]+L.*?(\d[\d\.,]+\b)', texto_norm),
        # Busca patrones como "TOTAL A PAGAR" o "IMPORTE"
        lambda: re.search(r'(?:TOTAL\s*A\s*PAGAR|IMPORTE).*?(\d[\d\.,]+)', texto_norm),
        # Busca el símbolo de moneda seguido de número
        lambda: re.search(r'\$\s*(\d[\d\.,]+)\b', texto_norm),
        # Devuelve el último número encontrado en el texto
        lambda: (lambda m: m[-1] if m else None)(re.findall(r'\b\d[\d\.,]+\b', texto_norm)),
        # Calcula la diferencia entre EFECTIVO y CAMBIO, si se encuentran
        ...
        # Toma el máximo de todos los números encontrados (suponiendo que el total es el mayor)
        lambda: max(
            [str_to_float(x) for x in re.findall(r'\b\d[\d\.,]+\b', texto_norm) if str_to_float(x) is not None],
            default=None
        ),
        # Alternativa: usar el último número de la lista
    ...
    total = None
    if valores_validos:
        # Selecciona el valor que aparezca con mayor frecuencia entre las estrategias
        total = max(set(valores_validos), key=valores_validos.count)
    
    # Estrategia adicional: fuzzy matching en líneas que contengan "TOTAL"
    if total is None:
